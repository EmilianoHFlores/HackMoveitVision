...
class SimplePubSub(Node):
    def __init__(self):
        ...
        self.depthsubscription = self.create_subscription(Image, '/head_front_camera/depth_registered/image_raw', self.depth_callback, 10)
        self.infosubscription = self.create_subscription(CameraInfo, '/head_front_camera/rgb/camera_info', self.info_callback, 1)
        self.pointPublisher = self.create_publisher(Point, 'ballTopic', 1)
        self.stampedPointPublisher = self.create_publisher(PointStamped, 'ballTopicStamped', 10)
        self.subscription
        self.br = CvBridge()
    ...
    def img_callback(self, data):
        ...
        try:
            ...
            if len(self.x_list) > 10:
                ...
                # evaluate at catch time
                estimated_x = model_tx[0]*catch_time + model_tx[1]
                estimated_y = model_ty[0]*catch_time**2 + model_ty[1]*catch_time + model_ty[2]
                estimated_z = model_tz[0]*catch_time + model_tz[1]
